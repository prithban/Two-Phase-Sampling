...
#########################
####Informants score invariant with time with fixed incidence rate (model 1).
#########################
rm(list=ls())
...
### Normal group
x<-as.vector(rnorm(900,2,2))    ##feature sample
...
### Removing patients in Demented group
...
#########################
####Informants score improves with time with fixed incidence rate (model 2).
#########################
rm(list=ls())
### Assign  baseline
m<-1 #baseline
...
for (i in 1:n1) {
  ind<- -1;
  while (ind<0)
  {if (temp[j,2]==2) {temp[j,1]<- rnorm(1,-2,3);  temp[j,2]<-1; ind<-1}; 
    j<- j+1;}
}    #newly demented people

### Improvement of information score
temp[,1]<-temp[,1]+((-1)^ temp[,2])*runif(1,0,2)*rbinom(1,1,0.7)
...
###########################
### Informants score degrades with time with fixed incidence rate (model 3).
###########################
rm(list = ls())
### Assign  baseline
m<-1 #baseline
...
temp[,1]<-temp[,1]+((-1)^(temp[,2]+1))*runif(1,0,2)*rbinom(1,1,0.5)
...
###########################
### Informants score invariant with time with variable incidence rate (model 4).
###########################
rm(list=ls())
set.seed(1000)
### Assign  baseline
...
### Normal group
x<-as.vector(rnorm(900,2,1.2))    ##feature sample
D<-rep(2,length(x))             ##response 
x1<-cbind(x,D)                  ##normal group data
...
### New patients in Demented group
theta<-runif(1,0,0.1); # variable incidence rate
...
###########################
### Informants score improves with time with variable incidence rate (model 5).
###########################
...
### New patients in Demented group
theta<-runif(1,0.01,0.1); # variable incidence rate
...
### Improvemnet of information score
temp[,1]<-temp[,1]+((-1)^ temp[,2])*runif(1,0,2)*rbinom(1,1,0.7)
